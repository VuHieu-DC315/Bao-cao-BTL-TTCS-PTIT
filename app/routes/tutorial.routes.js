...
module.exports = (app) => {
  ...
  router.post("/", checkAdmin, uploadProductImage, create);
  router.get("/:id", checkAdmin, findOne);
  router.put("/:id", checkAdmin, uploadProductImage, update);

  app.use("/admin/products", router);

  app.get("/product-images/:id", getImage);

  app.get("/shop", getHomesalePage);
  app.get("/products/:id", getBuyPage);
  app.post("/orders", buyTutorial); // xử lý mua hàng
