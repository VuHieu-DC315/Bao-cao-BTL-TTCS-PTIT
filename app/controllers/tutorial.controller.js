...
function buildImageFields(file) {
...
function sendFallbackProductImage(res, id) {
  const oldImagePath = path.join(__dirname, "../public/image", `${id}.jpg`);
  const defaultImagePath = path.join(__dirname, "../public/image/1.jpg");

  return res.sendFile(oldImagePath, (err) => {
...
module.exports = {
  ...
  update: async (req, res) => {
    try {
      ...
      Object.assign(dataToUpdate, buildImageFields(req.file));
  ...
  getImage: async (req, res) => {
    try {
      ...
      if (!tutorial || !tutorial.imageData) {
        return sendFallbackProductImage(res, id);
      }

      res.setHeader("Content-Type", tutorial.imageMimeType || "image/jpeg");
      res.setHeader("Cache-Control", "public, max-age=86400");
      return res.send(tutorial.imageData);
    } catch (error) {
      console.log("getImage error =", error);
      return sendFallbackProductImage(res, req.params.id);
    }
  },
